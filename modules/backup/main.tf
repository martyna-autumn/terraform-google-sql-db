...
data "google_sql_database_instance" "backup_instance" {
  count   = var.cloudsql_db_type != "" || var.cloudsql_sa_email != "" ? 0 : 1
...
resource "google_workflows_workflow" "sql_export" {
  ...
  source_contents = templatefile("${path.module}/templates/export.yaml.tftpl", {
    ...
    dbType                 = var.cloudsql_db_type != "" ? var.cloudsql_db_type : split("_", data.google_sql_database_instance.backup_instance.database_version)[0]
...
resource "google_storage_bucket_iam_member" "sql_instance_account" {
  ...
  member = var.cloudsql_sa_email != "" ? "serviceAccount:${var.cloudsql_sa_email}" : "serviceAccount:${data.google_sql_database_instance.backup_instance.service_account_email_address}"
